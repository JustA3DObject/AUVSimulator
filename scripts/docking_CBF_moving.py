...
# Coordinates of four cardinal green lights on the moving dock funnel
# These are given in the vehicle frame of the surface dock.
LIGHT_COORDS = [
    [-6.0, 2.56, 0.0],     # Light3: top
    [-6.0, 0.0, 2.41],     # Light1: right
    [-6.0, -2.56, 0.0],    # Light7: bottom
    [-6.0, 0.0, -2.41]     # Light5: left
]
    ...
    def image_cb(self, msg):
        ...
        for c in contours_green:
            if cv2.contourArea(c) > 2:
                ...
                if radius > 1:
                    green_centers.append((float(x), float(y), float(radius)))

        green_centers.sort(key=lambda item: item[2], reverse=True)
        top_greens = green_centers[:5]

        is_circular_pattern = False
        geom_cx, geom_cy = 0.0, 0.0

        if len(top_greens) >= 4:
            num_lights = len(top_greens)
            geom_cx = sum(pt[0] for pt in top_greens) / num_lights
            geom_cy = sum(pt[1] for pt in top_greens) / num_lights
            
            distances = [math.hypot(pt[0]-geom_cx, pt[1]-geom_cy) for pt in top_greens]
            avg_dist = sum(distances) / num_lights
            
            if avg_dist > 10.0:
                variance = sum((d - avg_dist)**2 for d in distances) / num_lights
                std_dev = math.sqrt(variance)
                
                tolerance = 0.35 if num_lights == 4 else 0.25
                
                if std_dev < (tolerance * avg_dist):
                    is_circular_pattern = True
                    
                    for pt in top_greens:
                        cv2.circle(cv_image, (int(pt[0]), int(pt[1])), int(pt[2]) + 3, (0, 255, 0), 2)
                    
                    cv2.circle(cv_image, (int(geom_cx), int(geom_cy)), int(avg_dist), (0, 255, 255), 2)
                    cv2.putText(cv_image, f"{num_lights}-LIGHT RING", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
...
class DockingGNC(Node):
    """
    The core Guidance, Navigation, and Control (GNC) node for the AUV.
    """
    def __init__(self):
        ...
        self.create_subscription(Odometry, '/surface_dock/dynamics/odometry', self.dock_cb, 1)
        ...
        self.get_logger().info("Docking Node Started. CBF + Vision Enabled. Moving Dock Configured.")
    ...
    def add_logger_subscribers(self):
        ...
        self.create_subscription(Odometry, '/girona500/dynamics/odometry',
                                 self.auv_cb, qos_profile_sensor_data)
        self.create_subscription(Odometry, '/surface_dock/dynamics/odometry',
                                 self.dock_cb, qos_profile_sensor_data)
        ...
        self.create_subscription(Float64MultiArray, '/vision/dock_features',
                                 self.vision_cb, 1)

    def dock_cb(self, msg):
        noisy_x = msg.pose.pose.position.x + np.random.normal(0, NOISE_DOCK_STD)
        noisy_y = msg.pose.pose.position.y + np.random.normal(0, NOISE_DOCK_STD)
        noisy_z = msg.pose.pose.position.z + np.random.normal(0, NOISE_DOCK_STD)
        
        self.dock_pose = msg.pose.pose
        self.dock_pose.position.x = noisy_x
        self.dock_pose.position.y = noisy_y
        self.dock_pose.position.z = noisy_z
        
        if hasattr(self, 'data_logger'):
            self.data_logger.log_val('Dock_Pos', [noisy_x, noisy_y, noisy_z])
    ...
    def auv_cb(self, msg):
        ...
        if dt > 0:
            ...
            if hasattr(self, 'data_logger'):
                self.data_logger.log_val('AUV_Pos', [self.auv_x, self.auv_y, self.auv_z])
    ...
    def control_loop(self):
        ...
        dock_vehicle_pos = np.array([self.dock_pose.position.x,
                                     self.dock_pose.position.y,
                                     self.dock_pose.position.z])
        R_dock = quaternion_matrix(self.dock_pose.orientation)

        P_tail_3d = dock_vehicle_pos + R_dock.dot(self.magnet_offset)
        P_mouth_3d = dock_vehicle_pos + R_dock.dot(self.light_center_local)

        P_tail_2d = P_tail_3d[:2]
        P_mouth_2d = P_mouth_3d[:2]
        dock_z_target = P_tail_3d[2]
        ...
        # --- GLOBAL RELATIVE CLOSING VELOCITY UPDATE ---
        if self.prev_dist_to_dock is None:
            self.prev_dist_to_dock = dist_to_dock
            
        raw_v_close = (self.prev_dist_to_dock - dist_to_dock) / dt
        self.prev_dist_to_dock = dist_to_dock
        
        # Clamp derivative noise spikes induced by gaussian odometry errors
        raw_v_close = max(min(raw_v_close, 1.5), -1.5)
        
        # Low-pass filter to smooth out Odometry/EKF derivative noise (Strong filter for UI stability)
        self.v_close_filtered = (0.95 * self.v_close_filtered) + (0.05 * raw_v_close)
        
        # Update the global telemetry so the dashboard accesses it immediately
        self.telemetry['v_close'] = self.v_close_filtered
        self.telemetry['dist'] = dist_to_dock

        # Calculate a unit vector pointing directly out of the dock mouth
        len_mouth = np.linalg.norm(P_mouth_3d - P_tail_3d)
        if len_mouth > 0.001:
            u_mouth_3d = (P_mouth_3d - P_tail_3d) / len_mouth
        else:
        ...
        if not self.reached_staging_point:
            if dist_to_staging < 2.0 and abs(depth_error) < 1.0:
                ...
                self.telemetry.update({
                    'phase': 'APF Staging', 'z_err': depth_error,
                ...
                self.get_logger().info(
                    ...
                    f"XTE: 0.00m | FwdSpeed: {self.auv_forward_speed:.2f}m/s | V-Close: {self.v_close_filtered:.2f}m/s",
                    throttle_duration_sec=0.2
                )
                return

        # PHASE 2: Terminal Docking
        ...
        raw_diff = yaw_error_deg - self.prev_yaw_error_deg
        delta_yaw = math.degrees(normalize_angle(math.radians(raw_diff)))
        yaw_error_dot_raw_deg = delta_yaw / dt
        self.prev_yaw_error_deg = yaw_error_deg

        if not hasattr(self, 'filtered_yaw_dot_pid'):
            self.filtered_yaw_dot_pid = 0.0
        self.filtered_yaw_dot_pid = (0.85 * self.filtered_yaw_dot_pid) + (0.15 * yaw_error_dot_raw_deg)

        self.integral_error += yaw_error_rad * dt
        self.integral_error = max(min(self.integral_error, INTEGRAL_LIMIT), -INTEGRAL_LIMIT)
        u_yaw_nom = (KP_HEADING * yaw_error_deg) + (KI_HEADING * self.integral_error) + (KD_HEADING * self.filtered_yaw_dot_pid)
        ...
        n_min, n_max = 3.0, 6.0
        r_d, f_dec, K_ps = 3.0, 7.0, 10.0
        K_is = 2.5 

        if not hasattr(self, 'speed_integral'):
            self.speed_integral = 0.0

        s_t = self.auv_forward_speed
        
        s_t_pos = max(0.0, self.v_close_filtered)
        d_t = dist_to_dock

        R_d_t = r_d + f_dec * (s_t_pos**2) / (2.0 * a_max)
        u_t = 0.0
        ...
        if d_t <= R_d_t:
            n_t = n_min + (n_max - n_min) * min(1.0, s_t_pos / s_c)
            safe_d_t = max(d_t, 0.001)
            
            if abs(R_d_t - r_d) < 1e-5:
                s_d_t = s_min
            else:
            ...
            e_s = s_d_t - self.v_close_filtered

            self.speed_integral += e_s * dt
            self.speed_integral = max(min(self.speed_integral, 1.5), -1.5)

            u_t = (K_ps * e_s) + (K_is * self.speed_integral) + s_d_dot
            
            u_surge = (-140.0 * (s_d_t / s_c)) + (-160.0 * u_t)
            u_surge *= speed_scale
        else:
            self.s_d_prev = None
            self.speed_integral = 0.0
        ...
        dist_to_goal = np.linalg.norm(P_goal - P_auv_2d)

        self.telemetry.update({
            'phase': 'PLATO Docking', 'z_err': depth_error,
            'yaw_err': yaw_error_deg, 'xte': body_sway_error, 'speed': self.auv_forward_speed,
            'rd': R_d_t, 'plato_u': u_t, 'dist_goal': dist_to_goal
        })

        self.get_logger().info(
            f"[PLATO Phase] Dist2Goal: {dist_to_goal:.2f}m | Dist: {dist_to_dock:.2f}m | "
            ...
            f"FwdSpeed: {self.auv_forward_speed:.2f}m/s | V-Close: {self.v_close_filtered:.2f}m/s | α: {alpha:.2f}",
            throttle_duration_sec=0.2
        )
...
def main(args=None):
    ...
    dock_path_line, = ax_map.plot([], [], [], 'r--', alpha=0.5, label='Dock Path')
    ...
    # Text indicator directly beneath the primary speed dial for Closing Velocity
    vclose_text = ax_dash.text(0, -0.35, "V-Close: 0.00 m/s", ha='center', fontsize=11, fontweight='bold', color='darkred')

    telemetry_box = ax_dash.text(0, -0.55, "", ha='center', va='top', fontsize=11, family='monospace',
    ...
    def update_plot(frame):
        if gnc_node.dock_pose is None:
            return path_line, dock_path_line, auv_dot, dock_dot, target_line, speed_needle, speed_text, vclose_text, telemetry_box, auv_cbf_dot

        curr_x = gnc_node.auv_x
        curr_y = gnc_node.auv_y
        curr_z = gnc_node.auv_z

        dock_vehicle_pos = np.array([gnc_node.dock_pose.position.x,
                                     gnc_node.dock_pose.position.y,
                                     gnc_node.dock_pose.position.z])
        R_dock = quaternion_matrix(gnc_node.dock_pose.orientation)
        magnet_offset = np.array([-1.2, 0.0, 0.0])
        dock_tail = dock_vehicle_pos + R_dock.dot(magnet_offset)
        dock_x, dock_y, dock_z = dock_tail
        ...
        if curr_x != 0.0 and curr_y != 0.0:
            ...
            dock_x_history.append(dock_x)
            dock_y_history.append(dock_y)
            dock_z_history.append(dock_z)

            path_line.set_data(x_history, y_history)
            path_line.set_3d_properties(z_history)
            
            dock_path_line.set_data(dock_x_history, dock_y_history)
            dock_path_line.set_3d_properties(dock_z_history)
            ...
            P_mouth_3d = dock_vehicle_pos + R_dock.dot(gnc_node.light_center_local)
            P_tail_2d = dock_tail[:2]
            P_mouth_2d = P_mouth_3d[:2]
            ...
            all_x = x_history + dock_x_history
            all_y = y_history + dock_y_history
            all_z = z_history + dock_z_history
            margin = 5.0
            ...
            # Update closing velocity display on UI
            vclose_text.set_text(f"V-Close: {t.get('v_close', 0.0):.2f} m/s")
            ...
            dash_str = (
                ...
                f"Rel Vel (V_c): {t.get('v_close', 0.0):6.2f} m/s\n"
