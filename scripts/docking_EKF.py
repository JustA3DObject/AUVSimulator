...
class DockingGNC(Node):
    def __init__(self):
        ...
        self.telemetry = {
            'dist': 0.0, 'z_err': 0.0, 'yaw_err': 0.0, 
        ...
        self.auv_ekf = SimpleEKF(q_pos=0.01, q_yaw=0.01, r_pos=NOISE_POS_STD, r_depth=NOISE_DEPTH_STD, r_yaw=NOISE_YAW_STD)
        ...
        self.get_logger().info("Docking Node Started with EKF enabled.")
    ...
    def control_loop(self):
        ...
        self.telemetry.update({
            ...
            'xte': body_sway_error,
            'speed': self.auv_forward_speed,
            'rd': R_d_t,
            'plato_u': u_t
        })

        self.get_logger().info(
            f"Dist: {dist_to_dock:.2f}m | Z-Err: {depth_error:.2f}m | YawErr: {yaw_error_deg:.1f}° | XTE: {body_sway_error:.2f}m | FwdSpeed: {self.auv_forward_speed:.2f}m/s | Rd: {R_d_t:.2f}m | PLATO_u: {u_t:.3f}",
...
class AUVLogger(Node):
    ...
    def log_val(self, name, val): self.write_line(name, val)
    ...
    def log_gps(self, msg): self.write_line("GPS", [msg.latitude, msg.longitude])
    def log_pressure(self, msg): self.write_line("Pressure", msg.fluid_pressure)
    ...
    def log_camera(self, msg): self.write_line("Main Camera", f"{msg.width}x{msg.height}")
    def log_fls(self, msg): self.write_line("FLS Sonar", f"{msg.width}x{msg.height}")
...
def main(args=None):
    ...
    # Start the ROS 2 node in a background thread
    ros_thread = threading.Thread(target=executor.spin, daemon=True)
    ros_thread.start()

    # Configure Matplotlib UI Layout
    ...
    ax_map.set_title("Live AUV 3D Trajectory & LOS Targeting")
    ...
    def update_plot(frame):
        ...
        if curr_x != 0.0 and curr_y != 0.0:
            ...
            # Dynamically pan the camera bounds for all three axes
